...
from tkinter import Menu, Label, Entry, Button, PhotoImage, LEFT, END, Toplevel, Frame, RAISED, Canvas, NW
import sqlite3
...
class App:
    def __init__(self, master):
        ...
        self.guestButton = Button(
            text='Connexion en tant que invité',
            image=self.guestImage,
            compound=LEFT,
            width=190,
            height=40,
    ...
    def login(self, event):
        ...
        if self.id == "" or self.password == "":
            tkinter.messagebox.showwarning(
                "SVP remplissez tout les champs manquants.")
        else:
            ...
            if self.db_pass == self.password:
                ...
                tkinter.messagebox.showerror(
                    "Connexion echouer",
                    "Login ou mot de passe invalide! SVP réessayer encore")
    ...
    def drawWin(self):
        ...
        if self.db_designation == 'System Administrator' or self.db_designation == 'Doctor':
            itemone.add_command(
                ...
                command=self.appointment)
            itemone.add_command(
                label='Modifier rendez-vous',
                command=self.update)
            itemone.add_command(
                label='Supprimer rendez-vous',
                command=self.delete)
        ...
        itemone.add_command(
            ...
            command=lambda: self.logout(top))
        ...
        self.userlogin = Label(
            self.right,
            text="Vous etes connecter en tant que:",
            font=('arial 12 bold'),
            fg='black')
    ...
    def aboutMaster(self):
        ...
        self.imgCanvas = Canvas(width=120, height=120)
    ...
    def reset_pass(self):
        ...
        def callback(*args):
            for i in range(
                    len(OptionList)):    # assign question number to for query in the database
        ...
        self.submit_answer = Button(self.secret_ques, text="Soumettre", font=(
            'arial 11'), width=12, height=2, command=self.subAnswerSecretQues)
        ...
        self.Sub_loginID = Button(
            self.otp,
            text="Soumettre",
            font=('arial 11'),
            width=12,
            command=self.subVeriEmail)
        ...
        # button to submit the answers
        self.submit_answer_otp = Button(
            self.otp,
            text="Soumettre",
            font=('arial 11'),
            width=12,
            command=self.subAnswerOTP)
    ...
    def subVeriEmail(self):
        ...
        if (is_connected(self)):
            ...
            self.emailStatus.configure(
                text='SVP patienté! envoie de email. . .', fg='black')
    ...
    def subAnswerSecretQues(self):
        tkinter.messagebox.showinfo(
            parent=self.top,
            title="Email envoyé",
            message="La vérification a été envoyée avec succès à votre adresse e-mail")
        ...
        if self.secret_status and self.secret_answer == self.ans:
            sql_pass_update_query = "UPDATE credentials SET pass=? where id=?"
            c.execute(
                sql_pass_update_query,
                (self.new_pass_ent.get(),
                 self.forgetID,
                 ))
            conn.commit()
        else:
...
def sendVeriEmail(self, name, email):
    ...
    <body>
        <p align="center" style="text-align: center;font-size: larger;">
            <img src="https://raw.githubusercontent.com/chauhannaman98/MedFixture/master/resources/icon.png"
        ...
        <h1 align="center">Hi! """ + name + """ Seems like you forgot your password. </h1>
            ...
            <p class="veri-code" style="text-align: center;font-size: larger;
            font-family: 'Inconsolata', monospace;">""" + x + """</p>
    </body>
    <footer style="text-align: center;">
        <ul style="list-style-type: none;padding: 0;margin: 0;" , margin="0," padding="0">
            <li style="font-size: smaller;">An open-source project maintained by
            <a href="https://github.com/chauhannaman98">
            chauhannaman98</a></li>
            <li style="font-size: smaller;"><a href="https://chauhannaman98.github.io/MedFixture">
            Visit website</a></li>
        </ul>
    </footer>
    </html>
    ...
    try:
        ...
        self.emailStatus.configure(
            text='Code sent to your registered email', fg='green')
